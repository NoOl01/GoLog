...
func writeLog(logType, logMessage string) {
	...
	name := logType + "." + time.Now().Format("2006-01-02.15.04")
	...
	case "info_log":
		{
			...
			if internal.Console {
				fmt.Printf("\u001B[37m[INFO]\u001B[0m: %s\n", logMessage)
			}
		}
	}
}

// ErrLog — outputs error logs to the console (if not disabled) and saves the logs to a new file with a date in the format "DD-MM-YYYY_hh.mm".
func ErrLog(err error) {
	writeLog("err_log", err.Error())
}

// WarnLog — outputs warnings logs to the console (if not disabled) and saves the logs to a new file with a date in the format "DD-MM-YYYY_hh.mm".
func WarnLog(warn string) {
	writeLog("warn_log", warn)
}

// InfoLog — outputs information logs to the console (if not disabled) and saves the logs to a new file with a date in the format "DD-MM-YYYY_hh.mm".
